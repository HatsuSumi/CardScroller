...
- **代码总行数**：27,661 行（不含空行、注释）
  - JavaScript：18,851 行（68.2%）
  - CSS：7,014 行（25.4%）
  - HTML：1,796 行（6.5%）

- **字符总数**：1,237,274 字符（不含注释）
  - JavaScript：936,354 字符（75.7%）
  - CSS：182,830 字符（14.8%）
  - HTML：118,090 字符（9.5%）
...
3. **UI/UX 动画效果**（33 种 CSS @keyframes 关键帧动画 + 175 个 transition 过渡 + 119 个 :hover 交互 + 5 个 SVG 动画 + 2 个 Canvas 动画 + 1 个 JS 数值动画）
   ...
   **CSS transition 过渡**（175 个）：
   ...
   - **其他**（24 个）：图片预览、进度倒计时、隐藏文本、折叠内容、滚动条等
   
   **CSS :hover 交互动画**（119 个）：
   ...
   - **链接/导航类**（12 个）：帮助链接、导航项、气泡菜单项、个人网站链接（颜色变化 + 下划线展开动画）等
